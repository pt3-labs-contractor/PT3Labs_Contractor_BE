...
async function checkRequisiteUserInfo(req, res, next){
  const user = await query('SELECT * FROM users WHERE id = $1;', [req.session.passport.user]);
  if (!user.rows[0]){
  ...
  if (!username || !phone_number || !email) res.status(400).json({ missing_information: Object.keys(requiredInfo) });
  else next();
}
